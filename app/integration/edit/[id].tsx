...
import { updateIntegration, getActiveJourneys, getIntegrations } from '@/lib/storage';
import type { Journey, Integration } from '@/lib/types';
import { COLORS, CATEGORY_DATA } from '@/lib/theme';

// ---- Progress dots indicator ----

function ProgressDots({ current, total = 2 }: { current: number; total?: number }) {
  return (
...
export default function IntegrationEditScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      (async () => {
        ...
        setNoteDate(integ.note_date);
        const catKey = getCategoryKey(integ.category);
        const q1 = (integ as any)[`${catKey}_q1`] || '';
        const q2 = (integ as any)[`${catKey}_q2`] || '';
        const q3 = (integ as any)[`${catKey}_q3`] || '';
        setAnswers([q1, q2, q3]);
        setFreeText(integ.free_text || '');
        setShowFreeText(!!integ.free_text);
        setCarryForward(integ.carry_forward || '');
        setLinkedJourneyId(integ.journey_id);
  ...
  async function handleSave() {
    ...
    const catKey = getCategoryKey(integration.category);
    const input: any = {
      note_date: noteDate,
    ...
    input[`${catKey}_q1`] = answers[0] || null;
    input[`${catKey}_q2`] = answers[1] || null;
    input[`${catKey}_q3`] = answers[2] || null;

    await updateIntegration(id, input);
    router.back();
  }
    ...
    return (
      <SafeAreaView style={s.safe} edges={['top']}>
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontSize: 15, color: COLORS.textTertiary }}>Loading…</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  if (!integration) {
    return (
      <SafeAreaView style={s.safe} edges={['top']}>
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontSize: 15, color: COLORS.textTertiary }}>Integration not found.</Text>
        </View>
      </SafeAreaView>
    );
  }

  const cat = integration.category.charAt(0).toUpperCase() + integration.category.slice(1).toLowerCase();
  const questions = CATEGORY_QUESTIONS[cat] ?? CATEGORY_QUESTIONS.Emotions;
  const accentColor = CATEGORY_DATA[cat]?.color ?? COLORS.accent;
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.topBar}>
        ...
        <Text style={[s.title, { color: accentColor }]}>Edit {cat}</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <TouchableOpacity
            style={[s.saveBtn, { backgroundColor: accentColor, marginTop: 24, marginBottom: Math.max(safeBottom + 32, 60) }]}
            onPress={handleSave}
            disabled={saving}
            activeOpacity={0.85}
          >
